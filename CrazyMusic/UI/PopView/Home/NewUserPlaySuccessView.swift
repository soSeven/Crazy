...
class NewUserPlaySuccessView: UIView {
    
    var action: (()->())?
    
    init(cash: Int, level: Int) {
        ...
        textLbl.width = width
        textLbl.height = 21.uiX
        textLbl.y = 72.5.uiX
        textLbl.attributedText = getAttributedText(n: cash)
        addSubview(textLbl)
        
        let lbl = UILabel()
        lbl.textColor = .init(hex: "#E1DAE6")
        ...
        lbl1.y = 142.5.uiX
        lbl1.x = 0
        lbl1.height = 13.5.uiX
        lbl1.width = width
        lbl1.attributedText = getNumberAttributedText(n: level)
        lbl1.textAlignment = .center
        addSubview(lbl1)
        
        let btnImg = UIImage.create("xsfl-jxcg-an")
        let btn = MusicButton()
    ...
    private func getAttributedText(n: Int) -> NSAttributedString {
        let att1: [NSAttributedString.Key:Any] = [
            .font: UIFont(style: .medium, size: 18.uiX),
        ...
        let a1 = NSMutableAttributedString(string: "获得现金奖励", attributes: att1)
    ...
    private func getNumberAttributedText(n: Int) -> NSAttributedString {
        let att1: [NSAttributedString.Key:Any] = [
            .font: UIFont(style: .regular, size: 14.uiX),
            .foregroundColor: UIColor(hex: "#E1DAE6")
        ]
        let att2: [NSAttributedString.Key: Any] = [
        ...
        let a1 = NSMutableAttributedString(string: "还差", attributes: att1)
        let a2 = NSAttributedString(string: "\(n)", attributes: att2)
        let a3 = NSAttributedString(string: "首歌曲", attributes: att1)
        a1.append(a2)
        a1.append(a3)
        return a1
    }
