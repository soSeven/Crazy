...
class SignPopCell2: CollectionViewCell {
    ...
    lazy var bgImgView: UIImageView = {
        let v = UIImageView(image: UIImage.create("qdtx-dhb-jrqd-bg3"))
        return v
    }()
    
    lazy var markImgView: UIImageView = {
        let v = UIImageView(image: UIImage.create("qdtx-gx-icon"))
        return v
    }()
    
    lazy var redBagImgView: UIImageView = {
        let v = UIImageView(image: UIImage.create("qdtx-wlhb-tb-icon2"))
        return v
    }()
    ...
    func bind(to model: SignListCellViewModel) {
        ...
        Observable.combineLatest(model.isToday.asObservable(), model.isActive.asObservable(), model.cash.asObservable()).subscribe(onNext: {[weak self] (isToday, isActive, cash) in
            ...
            if isToday > 0 {
                self.bgImgView.image = .create("qdtx-dhb-jrqd-bg")
                ...
                if cash > 0 {
                    ...
                    self.redBagImgView.image = .create("qdtx-wlhb-tb-icon2")
                }
            } else {
                if isActive > 0 {
                    self.bgImgView.image = .create("qdtx-dhb-jrqd-bg2")
                    self.titleLbl.textColor = .init(hex: "#FFFFFF")
                    self.numLbl.attributedText = self.getAttributedText(n: model.gold.value, isActive: true)
                } else {
                    self.bgImgView.image = .create("qdtx-dhb-jrqd-bg3")
                    self.titleLbl.textColor = .init(hex: "#FFFFFF")
                    self.numLbl.attributedText = self.getAttributedText(n: model.gold.value, isActive: false)
                }
                if cash > 0 {
            ...
            self.redBagImgView.snp.remakeConstraints { make in
                make.top.equalToSuperview().offset(33.5.uiX)
                make.centerX.equalToSuperview()
                make.size.equalTo(self.redBagImgView.image!.snpSize)
            }
