...
//            if UserManager.shared.isLogin || self.loading {
//                return
//            }
//            self.login()
...
//    }
    ...
    private func login() {
        ...
        zip.subscribe(onNext: {[weak self] u, ad in
            guard let self = self else { return }
            self.loading = false
    ...
    private func addAd() -> Single<BUSplashAdView?> {
        return Single<BUSplashAdView?>.create { [weak self] single -> Disposable in
            ...
            let ad = BUSplashAdView(slotID: "887374173", frame: .init(x: 0, y: 0, width: UIDevice.screenWidth, height: UIDevice.screenHeight - 97 - UIDevice.safeAreaBottom))
